...
当前版本已经接入免费优先的新闻/公告源聚合。行情部分仍使用 mock 数据，新闻和 SEC 公告会按配置实时拉取。
...
- `market_data_provider`：`news` 为 mock 行情 + 真实新闻/公告；`mock` 为纯模拟数据。
...
nasdaq_hotspot_agent/
  ...
  src/
    nasdaq_hotspot_agent/
      ...
      providers/
        base.py             # 数据源接口
        enriched.py         # mock 行情 + 新闻/公告增强 provider
...
1. 新闻线索：Marketaux、Alpha Vantage、Nasdaq RSS。它们主要提供标题、摘要、snippet、URL、发布时间、ticker/entity，不假设有完整正文。
2. 官方证据：SEC EDGAR。它提供 8-K、10-Q、10-K、S-1 等官方 filing，可信度最高。
...
- 行情仍是 mock 数据，新闻/公告已按配置接入。
- 不包含投资建议。
- 不做自动交易。
- Nasdaq-100 / QQQ 权重和个股涨跌幅后续仍需接真实行情 API。
